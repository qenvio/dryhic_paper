...
($\lambda_i'$). The model consists of 8 states that correspond to 1, 2, 3,
...
\textit{vanilla}, \textit{ICE} and \textit{oneD} corrections.
...
from T47D, the third contained all the mouse samples (see Supplementary
...
whereas the fourth method does it explicitly.
...
Figures~\ref{fig:aberrant}B), which is the desired behavior of a
...
(Figures~\ref{fig:aberrant}C and D). \textit{OneD} had the larest area
...
(Supplementary Figure~11). On this test, \textit{LGF} and \textit{OneD}
...
\textit{OneD} was the third fastest method and it always ran in less than
1~min in the conditions of the benchmark. The copy number correction
increased the running time, but it remained unert 1~min in general.
...
(Supplementary Figure~12). \textit{vanilla} was the overall fastest method
and \textit{OneD} was usually the faster than \textit{ICE} and \textit{KR}
