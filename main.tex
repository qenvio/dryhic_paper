...
Cuartero\,$^{\text{\sfb 1,2}}$, Thomas Graf\,$^{\text{\sfb 1,2}}$, Miguel
Beato\,$^{\text{\sfb 1,2}}$ and Guillaume J. Filion\,$^{\text{\sfb 1,2}}$}
...
\begin{table*}
\processtable{Hi-C data sets used in this study. Quique you're here.
\label{tab:samples}}
...
Spearman correlation (Supplementary Figure 2). All the methods except
...
(Supplementary Figures 6 and 7). \textit{OneD} was always among the top
...
to normalize data collected on samples with aberrant karyotypes, but it
...
does only corrects the copy number if specifically requested by the user.
Copy number variations treated as implicit biases have an
opaque effects on the 
...
should either be kept or removed. This is why \textit{OneD} does not perform
...
Overall, \textit{OneD} constitutes a safe choice to normalize Hi-C matrices. If the
