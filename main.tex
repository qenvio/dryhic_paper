...
{\begin{tabular}{ccccccc}
  ...
  \textbf{RE} & \textbf{Sequencing core} & \textbf{Set} &
  \textbf{Source} \\
  \midrule
...
The principle of OneD is to model explicit Hi-C biases onto a single
variable: the total amount of contacts for each bin of the matrix (see
section~\ref{sec:model}). The reason for this choice of that the total
amount of contacts is approximately proportional to the local copy number.
For instance, a duplicated region in a diploid genome will show on average
a 50\% increase in the number of contacts.
...
Other biases will also affect the total amount of contacts, though not
proportionately. Figure~\ref{fig:totals}A shows the relationship
between the amount of contacts and the availability of retriction enzyme
sites. Four clouds are visible. Each corresponds to a copy number
between one and four. In all of them, the relationship flattens as the
number of restriction sites increases. OneD thus fits a non-linear
relationship between the total amount of contacts and the predictors of
bias (by default the G+C content, the number of restriction sites and the
mappability of the reads).

The biases are estimated for each bin of the genome, and each cell of the
matrix is corrected using equation (\ref{eq:xhat}). The corrected amount
of contacts is still proportional to the copy number, but it is now less
noisy. The correction thus represents a more stable and precise estimation
of the copy number. Figure \ref{fig:totals}B shows the corrected number of
contacts along chromosme 17 of a T47D sample. The corrected contacts have
a narrower range than the raw contacts.
...
cell lines. OneD achieves the highest correlations in both cases.
Actually, it is the only method that increases the agreement with the
external data. The other methods, including caICB, only make the
normalized data more divergent than the raw counts.  Similar results were
obtained using Spearman correlation (Supplementary Figure 2).

Taken together, these results indicate that OneD is an efficient way to
estimate the local copy number in order to correct for it.
